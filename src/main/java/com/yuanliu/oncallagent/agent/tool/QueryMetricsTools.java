 ...
 * Description:
 * Prometheus 告警查询工具
 * 用于查询Prometheus的活动告警信息
 * @Author Yuan Liu
 * @Create 2026/6/1 10:44
 * @Version 1.0
 */
@Component
@Slf4j
public class QueryMetricsTools {
    ...
    public void init() {
        this.httpClient = new OkHttpClient.Builder()
                ...
                .build();
        log.info("✅ QueryMetricsTools 初始化成功, Prometheus URL: {}, Mock模式: {}", prometheusBaseUrl, mockEnabled);
    }

    /**
     * 查询Prometheus活动告警
     * 该工具从Prometheus告警系统检索所有当前活动/触发的告警，包括标签、注释、状态和值
     */
    ...
    public String queryPrometheusAlerts() {
        log.info("开始查询Prometheus 活动告警，Mock模式:{}",mockEnabled);
        try{
            List<SimplifiedAlert> simplifiedAlerts;
            if(mockEnabled){
                //=========== MOCK模式 ===========
                //开发/测试环境不依赖真实Prometheus，返回本地模拟数据
                //返回与文档关联的模拟告警数据
                simplifiedAlerts = buildMockAlerts();
                log.info("使用Mock数据，返回{}个模拟告警",simplifiedAlerts.size());
            }else{
                //================ 真实模式 ===============
                //调用真实的 Prometheus API 获取告警数据
                ...
                //转换为简化格式，对于相同的 alertname ，只保留第一个（去重）
                //例如：多个实例触发同一个告警，只返回第一个，避免信息冗余
            ...
            output.setMessage(String.format("成功检索到%d个活动告警",simplifiedAlerts.size()));
    ...
    private List<SimplifiedAlert> buildMockAlerts(){
        ...
        cpuAlert.setDescription("服务 payment-service 的 CPU使用率持续超过80%，当前值为92%" +
        ...
        memoryAlert.setDescription("服务 order-service 的内存使用率持续超过85%，当前值为91%。"+
                "JVM堆内存使用：3.8GB/4GB，可能存在内存泄露风险。"+
                "实例: pod-order-service-5c7d8e9f1-m3n2p，命令空间: production");
        ...
        slowAlert.setDescription("服务 user-service 的 P99 响应时间持续超过3秒，当前值为4.2秒。"+
    ...
    private String buildErrorResponse(String message, String error){
        try{
            ...
            //如果ObjectMapper序列化失败，返回最基础的JSON字符串
    ...
    public static class AlertsData{
        private List<PrometheusAlert> alerts = new ArrayList<>(); //告警列表
    }
