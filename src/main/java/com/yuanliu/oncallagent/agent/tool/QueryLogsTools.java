 ...
 * 日志查询工具
 * 用于查询CLS(云日志服务)的日志信息
 * 支持Mock模式，提供与告警关联的模拟日志数据
 * @Author Yuan Liu
 * @Create 2026/6/1 15:43
 * @Version 1.0
 */
@Component
@Slf4j
public class QueryLogsTools {
    ...
    public QueryLogsTools(QueryMetricsTools queryMetricsTools) {
        this.queryMetricsTools = queryMetricsTools;
    }

    @PostConstruct
    public void init(){
        log.info("✅ QueryLogsTools 初始化成功, Mock模式: {}", mockEnabled);
    }
    ...
    public String getAvailableLogTopics(){
        ...
        try{
            ...
            dbSlowQuery.setDescription("数据库慢查询日志，包含执行时间较长的SQL查询，可用于分析数据库性能问题");
            ...
            output.setMessage(String.format("共有 %d 个可用的日志主题。建议使用默认地域 'ap-guangzhou' 或者省略 region 参数",topics.size()));

            return objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(output);
        }catch (Exception e){
            log.error("获取日志主题列表失败",e);
    ...
    private static final String DEFUALT_REGION = "ap-guangzhou";
    ...
    "logTopics(required, one of the above topics or their CLS topicId),"+
    ...
    public String queryLogs(
           ...
           @ToolParam(description = "查询条件，支持 Lucene 语法，如 level:ERROR OR cpu_usage:>80；为空的时候返回该主题近5条核心日志") String query,
        ...
        try{
            ...
            if(mockEnabled){
                //Mock模式：返回与警告关联的模拟日志数据
                logEntries =buildMockLogs(region,logTopic,safeQuery,actualLimit);
                log.info("使用Mock数据，返回{}条日志",logEntries.size());
            }else{
                //真实模式：如果配置文件没有使用mock，则按照spring-ai-starter-mcp-client-webflux的规范自动注入mcp tool。不会走进来
                return buildErrorResponse("如果配置文件没有使用mock，则按照spring-ai-starter-mcp-client-webflux的规范自动注入mcp tool。不会走进来");
            }
            //构建成功响应
            ...
            log.info("日志查询完成：找到{}条日志",logEntries.size());
